    ...
    if (empty($email_register) && empty($name_register) && empty($pw_register) && ($pw_again)) {
        ...
        if (check_exist_email($connect, $email_register)) {
            ...
            if (sendToken($name_register, $email_register, $token)) {
                $sql_add = "INSERT INTO users(username,password,role,use_size,name,token) VALUES('" . $email_register . "','" . $password_register . "','1','0','" . $name_register . "','" . $token . "')";
                $query = mysqli_query($connect, $sql_add);
                // header("Location:login.php");
                if ($query) {

                    sendToken($name_register, $email_register, $token);
...
function sendToken($name, $email, $token)
{
    ...
    try {
        ...
        <p>Click vào<a href ='localhost/source/verify_user.php?email=$email&token=$token'> đây</a> để đăng nhập!</p>";
        $mail->send();
        return true;
    } catch (Exception $e) {
        echo $e;
